...
-- OBTENER FACTURAS MAYORES A UN PROMEDIO MÓVIL
SELECT *
FROM "Invoice"
WHERE "Total" > (
SELECT AVG("Total") AS "Promedio" -- se copió el código de búsqueda del promedio
FROM "Invoice"
)
...
-- ENCONTRAR CLIENTES (por ID) QUE TIENEN MÁS GASTO QUE EL PROMEDIO
SELECT "CustomerId", SUM ("Total") AS "ventas"
FROM "Invoice"
GROUP BY "CustomerId"
HAVING (SUM ("Total") > (
SELECT AVG("Total")
...
-- observación: no sirve el WHERE (por eso se usa el HAVING) dado que este operador no
-- funciona al haber "creado" una columna (la de GroupBy)
SELECT "CustomerId", SUM ("Total") AS "ventas"
FROM "Invoice"
GROUP BY "CustomerId"
WHERE "Total" > (
